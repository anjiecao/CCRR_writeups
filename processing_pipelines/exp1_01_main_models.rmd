...
saveRDS(full_b_rmts_model,here("cached_results/exp1/raw_models/bayes_models/full_b_rmts_model.Rds"))

saveRDS(null_b_rmts_model,here("cached_results/exp1/raw_models/bayes_models/null_b_rmts_model.Rds"))
...
saveRDS(full_b_rmts_model,here("cached_results/exp1/raw_models/bayes_models/full_b_fd_firstmention_model.Rds"))

saveRDS(null_b_rmts_model,here("cached_results/exp1/raw_models/bayes_models/null_b_fd_firstmention_model.Rds"))
```
...
```{r}
full_b_fd_descriptives_model <-  rstanarm::stan_glmer(
    description_num ~ description_type * culture + (description_type | subject) + (culture | scene), 
    data = fd_descriptive_df, 
    family = "gaussian",
    diagnostic_file = file.path(tempdir(), "df.csv"))

null_b_fd_descriptives_model <-  rstanarm::stan_glmer(
    description_num ~ description_type + culture + (description_type | subject) + (culture | scene), 
    data = fd_descriptive_df, 
    family = "gaussian",
    diagnostic_file = file.path(tempdir(), "df.csv"))

saveRDS(full_b_fd_descriptives_model, here("cached_results/exp1/raw_models/bayes_models/full_b_fd_descriptives_model.Rds"))
saveRDS(null_b_fd_descriptives_model, here("cached_results/exp1/raw_models/bayes_models/null_b_fd_descriptives_model.Rds"))
```
...
```{r}
full_b_ebb_model <-  rstanarm::stan_glmer(
...
null_b_ebb_model <-  rstanarm::stan_glmer(
...
saveRDS(full_b_ebb_model, here("cached_results/exp1/raw_models/bayes_models/full_b_ebb_model.Rds"))
saveRDS(null_b_ebb_model, here("cached_results/exp1/raw_models/bayes_models/null_b_ebb_model.Rds"))
```
...
```{r}
full_b_hz_height_model <-  rstanarm::stan_glmer(
    height ~ culture, 
    data = HZ_height_df, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

null_b_hz_height_model <-  rstanarm::stan_glmer(
    height ~ 1, 
    data = HZ_height_df, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

saveRDS(full_b_hz_height_model, here("cached_results/exp1/raw_models/bayes_models/full_b_hz_height_model.Rds"))
saveRDS(null_b_hz_height_model, 
        here("cached_results/exp1/raw_models/bayes_models/null_b_hz_height_model.Rds"))
```

#### n

```{r}
full_b_hz_n_model <-  rstanarm::stan_glmer(
    stkr_count ~ culture, 
    data = HZ_stkr_n_df, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

null_b_hz_n_model <-  rstanarm::stan_glmer(
    stkr_count ~ 1, 
    data = HZ_stkr_n_df, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

saveRDS(full_b_hz_n_model, here("cached_results/exp1/raw_models/bayes_models/full_b_hz_n_model.Rds"))
saveRDS(null_b_hz_n_model, 
       here("cached_results/exp1/raw_models/bayes_models/null_b_hz_n_model.Rds"))
```
...
```{r}
full_b_hz_area_model <-  rstanarm::stan_glmer(
    stkr_area ~ culture, 
    data = HZ_stkr_area_df, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

null_b_hz_area_model <-  rstanarm::stan_glmer(
    stkr_area ~ 1, 
    data = HZ_stkr_area_df, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

saveRDS(full_b_hz_area_model, here("cached_results/exp1/raw_models/bayes_models/full_b_hz_area_model.Rds"))
saveRDS(null_b_hz_area_model, 
       here("cached_results/exp1/raw_models/bayes_models/null_b_hz_area_model.Rds"))
```
...
```{r}
full_b_ssi_ratio_model <-  rstanarm::stan_glmer(
    score ~ culture, 
    data = si_df_r, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

null_b_ssi_ratio_model <-  rstanarm::stan_glmer(
    score ~ 1, 
    data = si_df_r, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

saveRDS(full_b_ssi_ratio_model, here("cached_results/exp1/raw_models/bayes_models/full_b_ssi_ratio_model.Rds"))

saveRDS(null_b_ssi_ratio_model, 
here("cached_results/exp1/raw_models/bayes_models/null_b_ssi_ratio_model.Rds"))
```

#### diff

```{r}
full_b_ssi_diff_model <-  rstanarm::stan_glmer(
    score ~ culture, 
    data = si_df_diff, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

null_b_ssi_diff_model <-  rstanarm::stan_glmer(
    score ~ 1, 
    data = si_df_diff, 
    diagnostic_file = file.path(tempdir(), "df.csv"))

saveRDS(full_b_ssi_diff_model, here("cached_results/exp1/raw_models/bayes_models/full_b_ssi_diff_model.Rds"))

saveRDS(null_b_ssi_diff_model, 
here("cached_results/exp1/raw_models/bayes_models/null_b_ssi_diff_model.Rds"))
```
...
```{r}
full_b_up_model <-  rstanarm::stan_glmer(
    choice ~ culture, 
    data = up_df, 
    family = "binomial",
    prior = normal(0,0.5),
...
null_b_up_model <-  rstanarm::stan_glmer(
    choice ~ 1, 
    data = up_df, 
    family = "binomial",
    prior = normal(0,0.5),
...
saveRDS(full_b_up_model, here("cached_results/exp1/raw_models/bayes_models/full_b_up_model.Rds"))

saveRDS(null_b_up_model, 
here("cached_results/exp1/raw_models/bayes_models/null_b_up_model.Rds"))
```
...
```{r}
full_b_ca_model <-  rstanarm::stan_glmer(
...
null_b_ca_model <-  rstanarm::stan_glmer(
    attrib_num ~ attrib_type + culture + (attrib_type | subject) + (culture | trial), 
    data = ca_df, 
    family = "poisson",
    prior = normal(3,0.5),
...
saveRDS(full_b_ca_model, here("cached_results/exp1/raw_models/bayes_models/full_b_ca_model.Rds"))

saveRDS(null_b_ca_model, 
here("cached_results/exp1/raw_models/bayes_models/null_b_ca_model.Rds"))
```
...
saveRDS(full_rv_model, here("cached_results/exp1/raw_models/bayes_models/full_b_rv_model.Rds"))

saveRDS(null_rv_model, 
here("cached_results/exp1/raw_models/bayes_models/null_b_rv_model.Rds"))
```
