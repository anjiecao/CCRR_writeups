...
```{r}
#BF analysis
full_b_fd_firstmention_model <- brm(first_mention ~ culture + (1 | subject)+(culture | scene) ,
                   save_pars = save_pars(all = TRUE), control = list(adapt_delta = .999, max_treedepth = 15),
                   iter = 5000, seed = 11111, data = fd_mention_df, 
                    file= here("cached_results/exp1/raw_models/bayes_models/full_b_fd_firstmention_model.Rds"),
                          cores=3)
null_b_fd_firstmention_model <- brm(first_mention ~ 1 + (1 | subject)+(culture | scene) ,
                   save_pars = save_pars(all = TRUE), control = list(adapt_delta = .999, max_treedepth = 15),
                   iter = 5000, seed = 11111, data = fd_mention_df, 
                    file= here("cached_results/exp1/raw_models/bayes_models/null_b_fd_firstmention_model.Rds"),
                          cores=3)

full_b_fd_descriptives_model <- brm(description_num ~ description_type * culture + (description_type | subject) + (culture | scene),
                             family = gaussian, control = list(adapt_delta = 0.995, max_treedepth = 15),
                             iter = 5000, seed = 11111, 
                             save_pars = save_pars(all = TRUE),
                             data = fd_descriptive_df, cores=3, 
                             file= here("cached_results/exp1/raw_models/bayes_models/full_b_fd_descriptives_model.Rds"))
  

# the null model for the interaction effect will contain the main effect of the model 
null_b_fd_descriptives_model <- brm(description_num ~ description_type + culture + (description_type | subject) + (culture | scene),
                             family = gaussian, control = list(adapt_delta = 0.995, max_treedepth = 15),
                             iter = 5000, seed = 11111, 
                             save_pars = save_pars(all = TRUE),
                             data = fd_descriptive_df, cores=3, 
...
full_b_ebb_model <- brm(correct ~ culture * context * size_diff + (size_diff * context | subject),
                        family = binomial, save_pars = save_pars(all = TRUE), iter = 5000, seed = 11111,
                         control = list(adapt_delta = 0.995, max_treedepth = 15),
                        data = ebb_df, cores=3, 
                        file = here("cached_results/exp1/raw_models/bayes_models/full_b_ebb_model.Rds"))

null_b_ebb_model <- brm(correct ~  context * size_diff + (size_diff * context | subject),
                        family = binomial, save_pars = save_pars(all = TRUE), iter = 5000, seed = 11111,
                         control = list(adapt_delta = 0.995, max_treedepth = 15),
                        data = ebb_df, cores=3, 
                        file = here("cached_results/exp1/raw_models/bayes_models/null_b_ebb_model.Rds"))
...
```{r}
# HZ_height BF 
full_b_hz_height_model <- brm(height ~ culture, family = gaussian, save_pars = save_pars(all = TRUE),
                              data = HZ_height_df, cores=3, 
                        file = here("cached_results/exp1/raw_models/bayes_models/full_b_hz_height_model.Rds"))


null_b_hz_height_model <- brm(height ~ 1, family = gaussian, save_pars = save_pars(all = TRUE),
                              data = HZ_height_df, cores=3, 
                        file = here("cached_results/exp1/raw_models/bayes_models/null_b_hz_height_model.Rds"))


#HZ_number BF analysis
full_b_hz_n_model <- brm(stkr_count ~ culture, family = gaussian,save_pars = save_pars(all = TRUE),
                         data = HZ_stkr_n_df, cores=3, 
                          file = here("cached_results/exp1/raw_models/bayes_models/full_b_hz_n_model.Rds"))

null_b_hz_n_model <- brm(stkr_count ~ culture, family = gaussian,save_pars = save_pars(all = TRUE),
                         data = HZ_stkr_n_df, cores=3, 
                          file = here("cached_results/exp1/raw_models/bayes_models/null_b_hz_n_model.Rds"))


full_b_hz_area_model <- brm(stkr_area ~ culture, family = gaussian, save_pars = save_pars(all = TRUE),
                            data = HZ_stkr_area_df, cores=3, 
                            file=here("cached_results/exp1/raw_models/bayes_models/full_b_hz_area_model.Rds"))

null_b_hz_area_model <- brm(stkr_area ~ 1, family = gaussian, save_pars = save_pars(all = TRUE),
                            data = HZ_stkr_area_df, cores=3, 
...
```{r}
full_b_ssi_ratio_model <- brm(score ~ culture, family = gaussian, save_pars = save_pars(all = TRUE),
                             data = si_df_r, cores=3,
                        file = here("cached_results/exp1/raw_models/bayes_models/full_b_ssi_ratio_model.Rds"))

null_b_ssi_ratio_model <- brm(score ~ 1, family = gaussian, save_pars = save_pars(all = TRUE),
                             data = si_df_r, cores=3, 
                         file = here("cached_results/exp1/raw_models/bayes_models/null_b_ssi_ratio_model.Rds"))


full_b_ssi_diff_model <- brm(score ~ culture, family = gaussian, save_pars = save_pars(all = TRUE),
                            data = si_df_diff, cores=3, 
                        file = here("cached_results/exp1/raw_models/bayes_models/full_b_ssi_diff_model.Rds"))

null_b_ssi_diff_model <- brm(score ~ 1, family = gaussian, save_pars = save_pars(all = TRUE),
                            data = si_df_diff, cores=3, 
...
```{r}
full_b_up_model <- brm(choice ~ culture, family = binomial(link="logit"), save_pars = save_pars(all = TRUE),
                       data = up_df, cores=3, 
                      file = here("cached_results/exp1/raw_models/bayes_models/full_b_up_model.Rds"))


null_b_up_model <- brm(choice ~ 1, family = binomial(link="logit"), save_pars = save_pars(all = TRUE),
                       data = up_df, cores=3, 
...
```{r}
#BF analysis, full model
# 12 divergent transitions
full_b_ca_model <- brm(attrib_num ~ attrib_type * culture + (attrib_type | subject) + (culture | trial),
                       family = poisson, control = list(adapt_delta = 0.999, max_treedepth = 15),
                       iter = 5000, seed = 11111,
                       save_pars = save_pars(all = TRUE), data = ca_df, cores=3, 
                      file = here("cached_results/exp1/raw_models/bayes_models/full_b_ca_model.Rds"))

null_b_ca_model <-  brm(attrib_num ~ attrib_type  + culture + (attrib_type | subject) + (culture | trial),
                       family = poisson, control = list(adapt_delta = 0.999, max_treedepth = 15),
                       iter = 5000, seed = 11111,
                       save_pars = save_pars(all = TRUE), data = ca_df, cores=3, 
...
```{r}
# #BF analysis
full_rv_model <- brm(acc ~ culture + (1 | subject) + (culture | trial), 
                     family = binomial(), save_pars = save_pars(all = TRUE),
