...
fd_description_df <- read_csv(here("data/03_processed_data/exp2/tidy_background.csv"))
fd_descriptive_model <- glmer(resp ~ resp_type * culture + (1 | subject) + (1 | trial_info), family=poisson, data = fd_description_df)
...
#### fd descriptive mod 
```{r}
fd_description_res <- tidy_model_main("fd_descriptive_model.Rds")
saveRDS(fd_description_res, here("cached_results/exp2/fd_descriptive_info.Rds"))
```
