...
tidy_d %>% 
  ...
  select(choice_causal, culture, subject, item) %>% 
  mutate(choice_causal = as.numeric(choice_causal)) %>% 
  ggplot(aes(x = culture, y = choice_causal, color = item)) + 
  #geom_point(alpha = .3) + 
...
full_b_SeI_model_pruned <- brm(choice_causal ~ culture + (1 | subject), 
                      family = binomial,
                       control = list(adapt_delta = .999, 
                                    max_treedepth = 15),
                     iter = 5000, seed = 11111,
                     save_pars = save_pars(all = TRUE), 
                      data = SeI_df, 
                     file = here("cached_results/exp2/raw_models/bayes_models/full_b_SeI_model_pruned.Rds"))

null_b_SeI_model_pruned <- brm(choice_causal ~ 1 + (1 | subject), 
                      family = binomial,
                       control = list(adapt_delta = .999, 
                                    max_treedepth = 15),
                     iter = 5000, seed = 11111,
                     save_pars = save_pars(all = TRUE), 
file = here("cached_results/exp2/raw_models/bayes_models/null_b_SeI_model_pruned.Rds"),
                      data = SeI_df)


full_b_SeI_model_culture_pruned <- brm(choice_causal ~ culture + (1 | subject) + (1 | item), 
                      family = binomial,
                       control = list(adapt_delta = .999, 
                                    max_treedepth = 15),
                     iter = 5000, seed = 11111,
                     save_pars = save_pars(all = TRUE), 
                      data = SeI_df, 
                     file = here("cached_results/exp2/raw_models/bayes_models/full_b_SeI_model_culture_pruned.Rds"))

null_b_SeI_model_culture_pruned <- brm(choice_causal ~ 1 + (1 | subject) + (1 | item), 
                      family = binomial,
                       control = list(adapt_delta = .999, 
                                    max_treedepth = 15),
                     iter = 5000, seed = 11111,
                     save_pars = save_pars(all = TRUE), 
file = here("cached_results/exp2/raw_models/bayes_models/null_b_SeI_model_culture_pruned.Rds"),
                      data = SeI_df)
